...
void StencilApp::OnKeyboardInput(const GameTimer& gt)
{
	...
	//Update shadow reflection world matrix
	XMStoreFloat4x4(&mShadowedReflectedSkullRitem->World, skullWorld * S * shadowOffsetY * R);

	mSkullRitem->NumFramesDirty = gNumFrameResources;
	mReflectedSkullRitem->NumFramesDirty = gNumFrameResources;
	mShadowedSkullRitem->NumFramesDirty = gNumFrameResources;
	mShadowedReflectedSkullRitem->NumFramesDirty = gNumFrameResources;
}
...
void StencilApp::BuildShadersAndInputLayout()
{
	...
	mShaders["standardVS"] = d3dUtil::CompileShader(L"Specific\\StencilDemo\\Shaders\\Main.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Specific\\StencilDemo\\Shaders\\Main.hlsl", defines, "PS", "ps_5_0");
	mShaders["alphaTestedPS"] = d3dUtil::CompileShader(L"Specific\\StencilDemo\\Shaders\\Main.hlsl", alphaTestDefines, "PS", "ps_5_0");
...
{
	std::ifstream fin("Specific/StencilDemo/Models/skull.txt");

	if (!fin)
	{
		MessageBox(0, L"Specific/StencilDemo/Models/skull.txt not found.", 0, 0);
		return;
	}
...
void StencilApp::BuildPSOs()
{
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC drawReflectedShadowsPsoDesc = transparentPsoDesc;
	drawReflectedShadowsPsoDesc.DepthStencilState = reflectedShadowDSS;
	drawReflectedShadowsPsoDesc.RasterizerState.CullMode = D3D12_CULL_MODE_BACK;
	drawReflectedShadowsPsoDesc.RasterizerState.FrontCounterClockwise = true;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&drawReflectedShadowsPsoDesc, IID_PPV_ARGS(&mPSOs["reflectedShadow"])));
}
...
void StencilApp::BuildRenderItems()
{
	...
	*reflectedFloorRitem = *floorRitem;
	...
	XMStoreFloat4x4(&reflectedFloorRitem->World, floorWorld * R);
	reflectedFloorRitem->ObjCBIndex = 6;
	mRitemLayer[(int)RenderLayer::Reflected].push_back(reflectedFloorRitem.get());

	//Shadowed reflected skull
	auto shadowedReflectedSkullRitem = std::make_unique<RenderItem>();
	*shadowedReflectedSkullRitem = *reflectedSkullRitem;
	shadowedReflectedSkullRitem->ObjCBIndex = 7;
	shadowedReflectedSkullRitem->Mat = mMaterials["shadowMat"].get();
	mShadowedReflectedSkullRitem = shadowedReflectedSkullRitem.get();
	mRitemLayer[(int)RenderLayer::ReflectedShadow].push_back(shadowedReflectedSkullRitem.get());
	...
	mAllRitems.push_back(std::move(shadowedReflectedSkullRitem));
